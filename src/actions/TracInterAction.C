...
registerMooseAction("BeaverApp", TracInterAction, "add_interface_kernel");
registerMooseAction("BeaverApp", TracInterAction, "add_material");
registerMooseAction("BeaverApp", TracInterAction, "add_master_action_material");
registerMooseAction("BeaverApp", TracInterAction, "add_aux_variable");
registerMooseAction("BeaverApp", TracInterAction, "add_aux_kernel");
registerMooseAction("BeaverApp", TracInterAction, "add_kernel");
registerMooseAction("BeaverApp", TracInterAction, "validate_coordinate_systems");

InputParameters
TracInterAction::validParams()
{
  InputParameters params = TracInterActionBase::validParams();
  params.addClassDescription("Action to create an instance of the VMNS "
                             "interface kernel for each displacement component.");
  params.addRequiredParam<std::vector<BoundaryName>>(
      "boundary", "The list of boundary IDs from the mesh where the cohesive zone will be applied");
  return params;
}

TracInterAction::TracInterAction(const InputParameters & params)
  : TracInterActionBase(params),
    ...
    _tracaver(getParam<MooseEnum>("trac_type").getEnum<TracAver>()),
    ...
    _verbose(getParam<bool>("verbose"))
{
  ...
  if (_ndisp > 3 || _ndisp < 1)
    mooseError("the VMNS Action requires 1, 2 or 3 displacement variables.");

  switch (_tracaver)
  {
    case TracAver::Simple:
    {
      _vmns_kernel_name = "VMNSInterfaceKernelSimple";
      _disp_jump_provider_name = "CZMComputeDisplacementJumpSmallStrain";
      _equilibrium_traction_calculator_name = "ComputeTractionAverJumpSimple";
      break;
    }
    case TracAver::Complex:
    {
      _vmns_kernel_name = "VMNSInterfaceKernelComplex";
      _disp_jump_provider_name = "CZMComputeDisplacementJumpSmallStrain";
      _equilibrium_traction_calculator_name = "ComputeTractionAverJumpSimple";
      break;
    }
    default:
      mooseError("TracInterAction Error: Invalid kernel parameter. Allowed values are: "
                 "Simple or Complex");
  }
...
void
TracInterAction::addRequiredVMNSInterfaceKernels()
{
  for (unsigned int i = 0; i < _ndisp; ++i)
  {
    // Create unique kernel name for each displacement component
    std::string unique_kernel_name = _vmns_kernel_name + "_" + _name + "_" + Moose::stringify(i);

    InputParameters paramsk = _factory.getValidParams(_vmns_kernel_name);
    ...
    paramsk.set<bool>("use_trac_penalty") = _use_trac_penalty;
    ...
    _problem->addInterfaceKernel(_vmns_kernel_name, unique_kernel_name, paramsk);
  }
}

void
TracInterAction::addRequiredVMNSInterfaceMaterials()
{
  ...
  // Create unique material name for the "ComputeTractionAverJump" object
...
TracInterAction::act()
{
  ...
  if (_current_task == "add_interface_kernel")
    addRequiredVMNSInterfaceKernels();
  else if (_current_task == "add_master_action_material")
    addRequiredVMNSInterfaceMaterials();
...
void
TracInterAction::addRelationshipManagers(Moose::RelationshipManagerType input_rm_type)
{
  InputParameters ips = _factory.getValidParams(_vmns_kernel_name);
  addRelationshipManagers(input_rm_type, ips);
}

void
TracInterAction::prepareSaveInInputs(std::vector<AuxVariableName> & save_in_names,
                                        std::string & save_in_side,
...
void
TracInterAction::verifyOrderAndFamilyOutputs()
{
...
void
TracInterAction::actOutputGeneration()
{
...
void
TracInterAction::actOutputMatProp()
{
  if (_current_task == "add_material")
  {
    ...
    for (auto out : _generate_output)
    {
          ...
          [&]()
          {
            for (const auto & vq : _real_vector_cartesian_component_table)
              for (unsigned int a = 0; a < 3; ++a)
                ...
                {
                  auto type = "VMNSRealVectorCartesianComponent";
                  ...
                  _problem->addInterfaceMaterial(type, _base_name + out + '_' + name(), params);
                  return true;
                }
            return false;
          }())
      ...
      if (setupOutput(out,
                      ...
                      {
                        auto type = "VMNSRealVectorScalar";
                        params = _factory.getValidParams(type);
                        ...
                        _problem->addInterfaceMaterial(
                            type, _base_name + out + '_' + name(), params);
                      }))
        continue;

      mooseError("VMNS Master: unable to add output Material");
    }
  }
}

void
TracInterAction::chekMultipleActionParameters()
{

  // Gather info about all other master actions when we add variables
  if (_current_task == "validate_coordinate_systems")
  {
    auto actions = _awh.getActions<TracInterAction>();
    for (const auto & action : actions)
    {
      ...
      if (size_after != size_before + added_size)
        mooseError("The boundary restrictions in the TracInterAction actions must be "
                   "non-overlapping.");
    }
    ...
    for (const auto & action : actions)
    {
      ...
      _trac_formulation_union.insert(action->_tracaver);
      const auto size_after = _trac_formulation_union.size();

      if (size_after != 1)
        mooseError("All blocks of the TracInterAction should have the same VMNS formulation");
    }
  }
}
