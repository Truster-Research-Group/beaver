...
const std::map<std::string, std::string>
    TracInterActionBase::_real_vector_cartesian_component_table = {
        {"trac_aver", "traction_aver_global"},
        {"jump", "displacement_jump_global"},
        {"trac_jump", "traction_jump_global"}};
...
InputParameters
TracInterActionBase::validParams()
{
  InputParameters params = Action::validParams();
  params.addClassDescription("Action to create an instance of the "
                             "VMNS interface kernel for each displacement component.");
  params.addRequiredParam<std::vector<VariableName>>(
      "displacements", "The nonlinear displacement variables for the problem");
  MooseEnum tracaverType("SIMPLE COMPLEX", "SIMPLE");
  params.addParam<MooseEnum>("trac_type", tracaverType, "Traction averaging type");
